...
class EditConvo extends Component {
  ...
  componentDidMount() {
    ...
    const convo = this.props.convos.convos.find(convo => {
      return convo._id === convoId;
    });
    this.setState({participants: convo.participants, title: convo.title, question: convo.question, schedule_days: convo.schedule_days});
  }
                ...
                <div className='select-time'>
                  <ButtonDropdown isOpen={this.state.timeDropdownOpen} toggle={() => { this.toggle('time') }}>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownItem active onClick={() => { this.handleTimeSelect('2:00') }}>2:00</DropdownItem>
